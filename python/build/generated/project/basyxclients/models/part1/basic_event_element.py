...
class BasicEventElement(BaseModel):
    ...
    direction: Direction
    state: StateOfEvent
    message_topic: Optional[Annotated[str, Field(min_length=1, strict=True, max_length=255)]] = Field(default=None, alias="messageTopic")
    message_broker: Optional[Reference] = Field(default=None, alias="messageBroker")
    last_update: Optional[Annotated[str, Field(strict=True)]] = Field(default=None, alias="lastUpdate")
    min_interval: Optional[Annotated[str, Field(strict=True)]] = Field(default=None, alias="minInterval")
    max_interval: Optional[Annotated[str, Field(strict=True)]] = Field(default=None, alias="maxInterval")
    __properties: ClassVar[List[str]] = ["extensions", "category", "idShort", "displayName", "description", "modelType", "semanticId", "supplementalSemanticIds", "qualifiers", "embeddedDataSpecifications", "observed", "direction", "state", "messageTopic", "messageBroker", "lastUpdate", "minInterval", "maxInterval"]
    ...
    def min_interval_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if value is None:
            return value

        if not re.match(r"^-?P((([0-9]+Y([0-9]+M)?([0-9]+D)?|([0-9]+M)([0-9]+D)?|([0-9]+D))(T(([0-9]+H)([0-9]+M)?([0-9]+(\.[0-9]+)?S)?|([0-9]+M)([0-9]+(\.[0-9]+)?S)?|([0-9]+(\.[0-9]+)?S)))?)|(T(([0-9]+H)([0-9]+M)?([0-9]+(\.[0-9]+)?S)?|([0-9]+M)([0-9]+(\.[0-9]+)?S)?|([0-9]+(\.[0-9]+)?S))))$", value):
            raise ValueError(r"must validate the regular expression /^-?P((([0-9]+Y([0-9]+M)?([0-9]+D)?|([0-9]+M)([0-9]+D)?|([0-9]+D))(T(([0-9]+H)([0-9]+M)?([0-9]+(\.[0-9]+)?S)?|([0-9]+M)([0-9]+(\.[0-9]+)?S)?|([0-9]+(\.[0-9]+)?S)))?)|(T(([0-9]+H)([0-9]+M)?([0-9]+(\.[0-9]+)?S)?|([0-9]+M)([0-9]+(\.[0-9]+)?S)?|([0-9]+(\.[0-9]+)?S))))$/")
        return value

    @field_validator('max_interval')
    def max_interval_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if value is None:
            return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.observed:
            _dict['observed'] = self.observed.to_dict()
        # override the default output from pydantic by calling `to_dict()` of message_broker
        if self.message_broker:
            _dict['messageBroker'] = self.message_broker.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "observed": Reference.from_dict(obj["observed"]) if obj.get("observed") is not None else None,
            "direction": obj.get("direction"),
            "state": obj.get("state"),
            "messageTopic": obj.get("messageTopic"),
            "messageBroker": Reference.from_dict(obj["messageBroker"]) if obj.get("messageBroker") is not None else None,
            "lastUpdate": obj.get("lastUpdate"),
            "minInterval": obj.get("minInterval"),
            "maxInterval": obj.get("maxInterval")
        })
        return _obj
