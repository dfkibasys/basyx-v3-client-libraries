...
public abstract class AbstractBasyxTest {
	...
	private static final String ENVIRONMENT_SUBMODELSERVICE_PORT = "environment.submodelservice.port";
	...
	static {
		...
		if (ENVIRONMENT_TYPE_EXTERNAL.equals(envType)) {
			...
			int smServicePort = getPort(ENVIRONMENT_SUBMODELSERVICE_PORT);
			...
			ENVIRONMENT = new BasyxTestEnvironmentExternal.Builder(MAPPER, host).aasRegistryPort(aasRegistryPort)
					...
					.smRepositoryPort(smRepositoryPort).smServicePort(smServicePort).build();
