...
import org.eclipse.digitaltwin.basyx.v3.clients.api.BasyxTestRunner.BasyxRepositoryState;
...
public class BasyxTestEnvironment {
	...
	private final GenericContainer<?> aasRegistryContainer = new GenericContainer<>(DockerImageName.parse("eclipsebasyx/aas-registry-log-mem:2.0.0-SNAPSHOT")).withExposedPorts(8080).withNetwork(NETWORK)
			.withNetworkAliases("aas-registry").withReuse(true);

	private final GenericContainer<?> smRegistryContainer = new GenericContainer<>(DockerImageName.parse("eclipsebasyx/submodel-registry-log-mem:2.0.0-SNAPSHOT")).withExposedPorts(8080)
			.withNetwork(NETWORK).withNetworkAliases("sm-registry").withReuse(true);
	private final GenericContainer<?> aasEnvironmentContainer = new GenericContainer<>(DockerImageName.parse("eclipsebasyx/aas-environment:2.0.0-SNAPSHOT")).withExposedPorts(8081)
			...
			.withEnv("BASYX_EXTERNALURL", "http://aas-environment:8081")
			.withNetwork(NETWORK).dependsOn(aasRegistryContainer, smRegistryContainer).withReuse(true);
	...
	public void initialize(BasyxRepositoryState initial) {
		try (CloseableHttpClient client = HttpClients.createMinimal()) {
			for (AssetAdministrationShell eachShell : initial.getShells()) {
				postShell(client, eachShell);
			}
			for (Submodel eachSm : initial.getSubmodels()) {
				postSubmodel(client, eachSm);
			}
		} catch (IOException e) {
	...
	public void cleanup() {
		try (CloseableHttpClient client = HttpClients.createMinimal()) {
			for (AssetAdministrationShell eachShell : getAllShells()) {
				deleteShell(client, eachShell.getId());
			}
	...
	private void deleteShellDescriptor(CloseableHttpClient client, String id) throws ClientProtocolException, IOException {
		deleteResource(client, aasRegistryContainer, "shell-descriptors", id);
	}
	
	private void deleteSubmodelDescriptor(CloseableHttpClient client, String id) throws ClientProtocolException, IOException {
	...
	private <T extends Identifiable> void deleteResource(CloseableHttpClient client, GenericContainer<?> container, String resourcePath, String id) throws ClientProtocolException, IOException {
		String idEncoded = Base64.getUrlEncoder().encodeToString(id.getBytes(StandardCharsets.UTF_8));
		HttpDelete delete = new HttpDelete("http://localhost:" + container.getFirstMappedPort() + "/" + resourcePath + "/" + idEncoded);
	...
	public List<AssetAdministrationShell> getAllShells() {
		return getAllResources(aasEnvironmentContainer.getFirstMappedPort(), "shells", GetAssetAdministrationShellsResult.class).getResult();
	}

	public List<Submodel> getAllSubmodels() {
		return getAllResources(aasEnvironmentContainer.getFirstMappedPort(), "submodels", GetSubmodelsResult.class).getResult();
	}

	public Collection<AssetAdministrationShellDescriptor> getAllShellDescriptors() {
		return getAllResources(aasRegistryContainer.getFirstMappedPort(), "shell-descriptors", GetAssetAdministrationShellDescriptorsResult.class).getResult();
	}

	public Collection<SubmodelDescriptor> getAllSubmodelDescriptors() {
		return getAllResources(smRegistryContainer.getFirstMappedPort(), "submodel-descriptors", GetSubmodelDescriptorsResult.class).getResult();
	}
	...
	private <T> T getAllResources(int port, String resourcePath, Class<T> cls) {
		try (CloseableHttpClient client = HttpClients.createMinimal()) {
			...
			try (CloseableHttpResponse response = client.execute(get)) {
				...
				try (InputStream in = response.getEntity().getContent(); BufferedInputStream bIn = new BufferedInputStream(in); InputStreamReader reader = new InputStreamReader(bIn, StandardCharsets.UTF_8)) {
	...
	public void postShell(CloseableHttpClient client, AssetAdministrationShell shell) throws ClientProtocolException, IOException {
		postResource(client, aasEnvironmentContainer, "shells", shell);
	}

	public void postShellDescriptor(CloseableHttpClient client, AssetAdministrationShellDescriptor descr) throws ClientProtocolException, IOException {
		postResource(client, aasRegistryContainer, "shell-descriptors", descr);		
	}
	
	public void postSubmodelDescriptor(CloseableHttpClient client, SubmodelDescriptor descr) throws ClientProtocolException, IOException {
		postResource(client, smRegistryContainer, "submodel-descriptors", descr);		
	}

	private void postResource(CloseableHttpClient client, GenericContainer<?> container, String path, Object resource) throws ClientProtocolException, IOException {
